...
(defn fannkuch-mt [n ntasks]
  ...
  (def to-child (ev/thread-chan ntasks))
  (def to-parent (ev/thread-chan ntasks))
  (for ti 0 ntasks
    (ev/spawn-thread
      (def rev-tab (ev/take to-child))
      (def res (fannkuch-task n (* tasksize ti) tasksize rev-tab))
      (ev/give to-parent res))
    (ev/give to-child rev-tab))
  (def res
